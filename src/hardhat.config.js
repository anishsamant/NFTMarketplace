...
const getAmbEndpoint = () => {
  console.log(process.env.NFT_HTTP_ENDPOINT)
  if (!process.env.NFT_HTTP_ENDPOINT) {
...
\`\`\`
export NFT_HTTP_ENDPOINT=https://<node id>.ethereum.managedblockchain.<region>.amazonaws.com
\`\`\`
`)
  }
  // The AWS console copies the endpoint without the required https:// prefix
  if (!process.env.NFT_HTTP_ENDPOINT.startsWith('https://')) {
    // Add https:// prefix if it doesn't already exist
    console.debug('Adding https:// prefix to AMB_HTTP_ENDPOINT');
    return `https://${process.env.NFT_HTTP_ENDPOINT}`;
  } else {
    return process.env.NFT_HTTP_ENDPOINT;
  }
}
...
module.exports = {
  ...
  networks: {
    hardhat: {},
    goerli: {
      url: process.env.NFT_HTTP_ENDPOINT || 'dummy',
    },
  },
};